...
def compute_xi_forest_pairs(xcf,x1,y1,z1,weights1,delta1,x2,y2,z2,weights2):
    ...
    bins_r = ((r-xcf["r_min"])/(xcf["r_max"]-xcf["r_min"])*xcf["nbins_r"]).astype(int)
    bins_mu = ((mu-xcf["mu_min"])/(xcf["mu_max"]-xcf["mu_min"])*xcf["nbins_mu"]).astype(int)
    ...
    return (rebin_weight, rebin_xi, rebin_r, rebin_mu, rebin_z,
            rebin_num_pairs)


def compute_xi_forest_pairs_rprt(xcf,x1,y1,z1,weights1,delta1,x2,y2,z2,weights2):
    ...
    bins_rp = ((rp-xcf["rp_min"])/(xcf["rp_max"]-xcf["rp_min"])*xcf["nbins_rp"]).astype(int)
    ...
    return (rebin_weight, rebin_xi, rebin_rp, rebin_rt, rebin_z,
            rebin_num_pairs)
...
def xcorr_cartesian_mine(xcf,save_corr=None):
    ...
    for d in deltas:
        ...
        if (len(voids) != 0):
            ...
            rt = np.sqrt((x_del[:,None] - x_voids)**2 + (y_del[:,None] - y_voids)**2)
            rp = -(z_del[:,None] - z_voids)
            r.append(np.ravel(np.sqrt(rp**2+rt**2)))
            mu.append(np.ravel(rp/r))
            xi.append(np.ravel((weights_deltas * delta_values)[:, None] * weights_voids))
    ...
    xi_array, r_array, mu_array, bc = binned_statistic_2d(r, mu, xi, "mean", bins=[nr,nmu])
    ...
    for d in deltas:
        ...
        if (len(voids) != 0):
            ...
            (rebin_weight,
             ...
             rebin_mu,
             rebin_z,
             rebin_num_pairs) =  compute_xi_forest_pairs(xcf,
                                                         x_del,
                                                         y_del,
                                                         z_del,
                                                         weights_deltas,
...
def xcorr_cartesian_rprt(xcf,save_corr=None):
    ...
    for d in deltas:
        ...
        if (len(voids) != 0):
            ...
            z_voids = voids[:,2]
            weights_voids = voids[:,3]
            (rebin_weight, rebin_xi, rebin_rp, rebin_rt, rebin_z, rebin_num_pairs) =  compute_xi_forest_pairs_rprt(xcf,x_del,y_del,z_del,weights_deltas,delta_values,x_voids,y_voids,z_voids,weights_voids)
            xi[:len(rebin_xi)]+=rebin_xi
            weights[:len(rebin_weight)]+=rebin_weight
    ...
    if(save_corr is not None):
        xcorr = xcorr_objects.CrossCorr(name=save_corr,mu_array=rt,r_array=rp,xi_array=xi,z_array=z,exported=True,rmu=False)
        xcorr.write()
...
def xcorr(delta_path,void_catalog,xcorr_options,corr_name,error_calculation=None,rtrp=False):
