...
class CrossCorr(object):
    ...
    def plot_2d_regrid(self,name,x_array,y_array,xi_array,xlabel,ylabel,vmin,vmax,colobar_legend):
        ...
        grid = griddata(np.array([np.ravel(x_array),np.ravel(y_array)]).T,
                        np.ravel(xi_array),
                        (xx, yy),
                        method='nearest')
...
class Multipole(object):
    ...
    @classmethod
    def init_from_xcorr(cls,ell,mu,xi,method,r_array=None,name=None,extrapolate=True):
        poles = {}
        for i in range(len(ell)):
            poles[ell[i]] = Multipole.get_multipole_from_array(xi,mu,ell[i],method,extrapolate_mu=extrapolate)
        return(cls(name=name,
                   r_array=r_array,
                   ell=ell,
                   poles=poles))
    ...
    def get_multipole_from_array(xi,mu,order,method,extrapolate_mu=True):
        ...
        if(method=="rect"):
            ...
            pole_l = Multipole.get_multipole_from_array_rebin_sample(xi,mu,order,method.split("_")[1])
