...
class DeltaGenerator(object):
    ...
    def create_radecz_array_middle(self):
        ...
        ra_array,dec_array,redshift_array = utils.convert_cartesian_to_sky(x_array_mpc,y_array_mpc,z_array_mpc,
    ...
    def select_deltas_others(self):
        ...
        indice_box[:,:,:,0],indice_box[:,:,:,1],indice_box[:,:,:,2]= utils.convert_sky_to_cartesian(ra_array,dec_array,redshift_array,
                                                                           self.mode,inv_rcomov=inv_rcomov,inv_distang=inv_distang,
                                                                           distang=distang,suplementary_parameters=None)
        indice_box = indice_box/self.tomographic_map.mpc_per_pixel
        indice_box = indice_box.reshape(indice_box.shape[0]*indice_box.shape[1]*indice_box.shape[2],indice_box.shape[3])

        z_fake_qso = np.max(redshift_array)
        deltas = map_coordinates(map_3d, np.transpose(indice_box), order=1).reshape((indice.shape[0],indice.shape[1],indice.shape[2]))
    ...
    def create_radecz_array(self):
        ...
        indice = np.transpose(np.indices(self.tomographic_map.shape),axes=(1,2,3,0))*self.tomographic_map.mpc_per_pixel
        indice = indice + [minx,miny,minz]
        (rcomov,distang,inv_rcomov,inv_distang) = utils.get_cosmo_function(self.tomographic_map.Omega_m)
        indice[:,:,:,0],indice[:,:,:,1],indice[:,:,:,2] = utils.convert_cartesian_to_sky(indice[:,:,:,0],indice[:,:,:,1],indice[:,:,:,2],
                                                                                         self.mode,inv_rcomov=inv_rcomov,inv_distang=inv_distang,
                                                                                         distang=distang,suplementary_parameters=None)

        ramin,ramax = np.min(indice[:,:,:,0]),np.max(indice[:,:,:,0])
        decmin,decmax = np.min(indice[:,:,:,1]),np.max(indice[:,:,:,1])
        redshiftmin,redshiftmax = np.min(indice[:,:,:,2]),np.max(indice[:,:,:,2])
        if(self.los_selection["method"] == "equidistant"):
    ...
    def create_index_arrays_cartesian(self ,method ="mean"):
        ...
        (rcomov,distang,inv_rcomov,inv_distang) = utils.get_cosmo_function(self.tomographic_map.Omega_m)
        redshift_array = utils.convert_z_cartesian_to_sky_middle(z_array_mpc,inv_rcomov)
